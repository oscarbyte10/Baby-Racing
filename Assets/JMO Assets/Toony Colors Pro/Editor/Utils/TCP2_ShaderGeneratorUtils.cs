...
public static class TCP2_ShaderGeneratorUtils
{
		...
		get
		{
			if (!CustomOutputDir)
			{
				...
				if(!Directory.Exists(OUTPUT_PATH))
				{
					...
					{
						return rootPath + "/Shaders 2.0 Generated/";
					}
				}
	...
	private static bool EvaluateExpression( int lineNumber, List<string> features, params string[] conditions )
	{
		...
		try
		{
			TCP2_ExpressionParser.ExpressionLeaf.EvaluateFunction evalFunc = s => HasEntry(features, s);
	...
	private static string GetIncludeRelativePath(TCP2_Config config, Shader existingShader)
	{
		...
		if (outputPath == OUTPUT_PATH)
			return INCLUDE_REL_PATH;
		var tcp2includeFile = Directory.GetFiles(Application.dataPath, "TCP2_Include.cginc", SearchOption.AllDirectories);
		if (tcp2includeFile.Length == 1)
		{
			var absoluteTcp2IncludeDir = Path.GetDirectoryName(tcp2includeFile[0]) + "/";
